 ...
 */
	public static function extension($check, $deny = false, $allow = true) {
		...
		if ($deny === true || (is_array($deny) && self::validationExtension($check, $deny))) {
			return false;
		}
		if ($allow !== true && (is_array($allow) && !self::validationExtension($check, $allow))) {
			return false;
		}
		return true;
	}
...
//The old 2.0 version of this function actually works 
/**
 ...
 */
	public static function validationExtension($check, $extensions = array('gif', 'jpeg', 'png', 'jpg')) {
		if (is_array($check)) {
