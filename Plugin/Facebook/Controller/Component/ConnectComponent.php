	...
	private function __syncFacebookUser(){
		...
		if($Auth->user('id') && ($Auth->user('type') != 'guest')){
			...
			elseif(empty($this->authUser) && $this->createUser) {
				...
				//$this->log("no account - create password");
				
				
				if($this->__runCallback('beforeFacebookSave')){
					//debug($this->authUser);
					
					$existingUser = CakeSession::read('Auth.User');
					$authUser = $this->authUser['User'];
					
					$this->authUser['User'] = array_merge($existingUser, $authUser);
					
					//only save if an existing account was retrieved that we can attach the facebook id to
					if (!empty($this->authUser['User']['id']))
					{
